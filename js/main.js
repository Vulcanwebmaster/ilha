...
mit.main = function() {
  ...
  */
  ...
  try {
    ...
    if (mit.highScore)
      ui.high_score.text("Melhor Pontuação: "+ mit.highScore);
  ...
  */
  mit.gameOver = function() {
    ...
    // Show last_score
    ui.last_score.text("Ultima Pontuação: " + parseInt(mit.score));


    ui.start_game.html('re-start');
    ui.tweet.html('tweet Record');
    ui.fb.html('postar no Fb');
    ...
    // Share
    var tweet = document.getElementById("tweet");
    tweet.href='http://twitter.com/share?url=http://xmu.com.br/chamabala/&text=Eu fiz ' +data.rewards[0].type+ ' pontos no CHAMBALÁ, você consegui fazer melhor?&count=horiztonal&via=_rishabhp&related=solitarydesigns';
  
    var facebook = document.getElementById("fb");
    facebook.href='http://facebook.com/sharer.php?s=100&p[url]=http://xmu.com.br/chambala/&p[title]=Eu fiz ' +data.rewards[0].type+ ' pontos no CHAMBALÁ, você consegui fazer melhor?';

  };

  mit.last_time = new Date();
  setInterval(function() {
    mit.ui.fps_count.html(mit.fps.toFixed(0) + ' Frames');
  }, 1000);
